	...
	if (validationError) {
		...
		if (license.site_count !== undefined && license.license_limit !== undefined) {
			...
			if (limit === 0 || license.activations_left === 'unlimited') {
				listItems.push(<li key="activations">✓ {__('Unlimited Activations', 'arts-license-pro')}</li>)
			} else {
				listItems.push(
					<li key="activations">
		...
		helpContent = (
			<span>
				...
				<ProBadge showWrapper={false} renderAsLink={true} href={licenseConfig.purchaseUrl} text={__('Get License Key', 'arts-license-pro')} />
			</span>
		)
	}
